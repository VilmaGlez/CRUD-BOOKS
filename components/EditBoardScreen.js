...
class EditBoardScreen extends Component {
 ...
 // obtener el documento utilizando route.params.boardkey
    ...
    const dbRef = firebase.firestore().collection('boards').doc(this.props.route.params.boardkey)
    dbRef.get().then((doc) => {
     if (doc.exists) {
  ...
  updateBoard() {
    ...
    const updateDBRef = firebase.firestore().collection('boards').doc(this.state.key);
    updateDBRef.set({
